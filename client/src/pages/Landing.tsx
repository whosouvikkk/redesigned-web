...
export default function Landing() {
  return (
    <div className="min-h-screen bg-black text-gray-200 selection:bg-pink-500/30 selection:text-white relative overflow-hidden font-sans">
      ...
      {/* 2. REAL LOOKUP EXAMPLES (UPDATED TO 5 CARDS) */}
      <section className="py-24 px-6 max-w-7xl mx-auto z-10 relative">
        ...
        >
          <div className="max-w-2xl">
            ...
            <p className="text-gray-400 font-light text-lg">Sample sanitized payloads generated by our backend. 5 key modules optimized for instant resolution.</p>
          </div>
        </motion.div>

        {/* Flex layout centers the 5 cards perfectly (3 on top, 2 on bottom) */}
        <div className="flex flex-wrap justify-center gap-6 max-w-6xl mx-auto">
          <SampleCard 
            index={0} 
            icon={<Smartphone />} 
            title="Number Lookup" 
            subtitle="Telecom & Demographics" 
            data={[
              { label: 'Name', value: 'Nikhil Nayek' }, 
              { label: 'Mobile', value: '7980346129' }, 
              { label: 'Circle', value: 'JIO KOL' }, 
              { label: 'Alt No', value: '9007746058' },
              { label: 'ID', value: '[Redacted]' }
            ]} 
          />
          <SampleCard 
            index={1} 
            ...
            data={[
              { label: 'Name', value: 'NIKHIL NAYEK' }, 
              { label: 'Father', value: 'ABAANI NAYEK' }, 
              { label: 'Mobile', value: '9775554747' }, 
              { label: 'Circle', value: 'WEST BENGAL' },
              { label: 'Aadhaar', value: '[Redacted]' }
            ]} 
          />
          <SampleCard 
            index={2} 
            ...
            subtitle="RTO & Ownership Specs" 
            data={[
              { label: 'Reg No', value: 'KL41V3504' }, 
              { label: 'Owner', value: 'MUHAMMED IQBAL T A' }, 
              { label: 'Model', value: 'SPLENDOR PLUS' }, 
              { label: 'Financier', value: 'TATA CAPITAL LTD' },
              { label: 'RTO', value: 'KL41 Aluva' }
            ]} 
          />
          <SampleCard 
            index={3} 
            ...
            subtitle="Contact Extraction" 
            data={[
              { label: 'Vehicle', value: 'KL41V3504' }, 
              { label: 'Mobile', value: '8129199443' }, 
              { label: 'Chassis', value: 'MBLHAW222R...56' }, 
              { label: 'Engine', value: 'HA11E7R5G...37' },
              { label: 'Trace Status', value: 'Matched' }
            ]} 
          />
          <SampleCard 
            index={4} 
            ...
            data={[
              { label: 'Target', value: '7980346129' }, 
              { label: 'Rounds', value: '1' }, 
              { label: 'Status', value: 'Active / Ready' }, 
              { label: 'Thread', value: 'Background' },
...
// Updated SampleCard: Dynamic width logic for 5-card centering layout
function SampleCard({ icon, title, subtitle, data, index }: { icon: React.ReactNode; title: string; subtitle: string; data: { label: string; value: string }[]; index: number }) {
  return (
    ...
    >
      ...
      <div className="space-y-1 font-mono text-xs relative z-10 flex-1 flex flex-col justify-end">
        {data.map((item, i) => (
          <div key={i} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0 last:pb-0">
            <span className="text-gray-500">{item.label}</span>
            <span className="text-gray-200 font-medium text-right max-w-[150px] truncate">{item.value}</span>
          </div>
        ))}
      </div>
    </motion.div>
